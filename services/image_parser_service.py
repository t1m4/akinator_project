...
class WebDriver:
    ...
    def get_search_page(self, search_name):
        ...
        register_button = self.driver.find_element(
            by=By.CLASS_NAME, value="vector-search-box-input"
        )
        ...
        WebDriverWait(self.driver, 30).until(
            expected_conditions.presence_of_element_located(
                (By.CLASS_NAME, "suggestions-results")
            )
        )
    ...
    def get_photo_from_page(self):
        first_headline = self.driver.find_element(
            by=By.CLASS_NAME, value="mw-first-heading"
        )
        main_photo_container = self.driver.find_element(
            by=By.CLASS_NAME, value="infobox-image"
        )
    ...
    def get_from_search_results(self):
        search_results = self.driver.find_elements(
            by=By.CLASS_NAME, value="mw-search-result-heading"
        )
        results_urls = []
